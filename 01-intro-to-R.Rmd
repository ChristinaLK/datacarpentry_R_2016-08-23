...
### Zero-dimensional objects - using single values

Simple R objects include numbers, and text.  They look like this: 
...
We can perform simple calculations with number objects, and other 
operations with text.  

We can see the "type" of object by using the following function: 
...
`class()` is a "function". A function does something, and "returns" some
result.

The value inside the parentheses is called the function "argument": the
function's "input".

### Getting help

If you type `class` and pause for a moment, you'll get a pop-up with
...
### One-dimensional objects - using vectors
...
Here, we are using the `c()` (for concatenate) function to create the 
vector.  
...
columns are vectors that all have the same length.  It is the main format used 
for data anlysis in R.  You can create one yourself 
by using the `data.frame()` function and giving it several vectors that all have 
the same length.  However, we will usually create dataframes by reading in a table of  
data from a spreadsheet and this will be created as a dataframe object.  
